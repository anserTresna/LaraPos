...
const searchInput = document.getElementById('tblTransaksi');
const searchResults = document.getElementById('tblTransaksi');

searchInput.addEventListener('input', function () {
    const query = searchInput.value.trim();

    if (query.length > 0) {
        fetch(`/cart?query=${query}`)
            .then(response => response.json())
            .then(data => {
                searchResults.innerHTML = '';
                data.forEach(product => {
                    const li = document.createElement('li');
                    li.textContent = `${product.nama} - Rp ${product.harga} - Stok: ${product.jumlah}`;
